...
var BUNDLE_PACKAGE_NAME = "dsh-autogate";
...
function modelsFromCatalog(catalog, provider) {
  const id = String(provider);
  if (id === "" || catalog === null || typeof catalog !== "object") return [];
  const groups = catalog.groups;
  if (!Array.isArray(groups)) return [];
  const group = groups.find((candidate) => candidate !== null && typeof candidate === "object" && String(candidate.id) === id);
  if (group === void 0 || !Array.isArray(group.models)) return [];
  return group.models.map((model) => model?.id).filter((modelId) => typeof modelId === "string" && modelId !== "");
}
var CardForm = class {
  scope;
  specs;
  ...
  shell() {
    ...
    return {
      available: snapshot2.status === "ready",
      writable: snapshot2.writable,
  ...
  field(field) {
    ...
    const snapshot2 = this.scope.getSnapshot();
    const value = snapshot2.value?.[field];
    const user = snapshot2.user;
    ...
    if (staged.clear) {
      const inherited = snapshot2.inherited?.[field] ?? value;
  ...
  fieldChanged(field, staged) {
    const snapshot2 = this.scope.getSnapshot();
    if (staged.clear) {
      return snapshot2.user !== void 0 && Object.hasOwn(snapshot2.user, field);
    }
    const parsed = this.specs.get(field)?.parse(staged.text);
    if (parsed === void 0 || parsed.kind !== "set") return true;
    return !Object.is(parsed.value, snapshot2.value?.[field]);
  }
...
var TRAIL_ENDPOINT = "/api/autogate/trail";
function isTrailRecords(value) {
  return value !== null && typeof value === "object" && value.ok === true && Array.isArray(value.value);
}
var TrailController = class {
  ...
  unsubscribeSettings;
  unsubscribeSessions;
  constructor(rpc, settings, uiSession) {
    this.rpc = rpc;
...
var zh = {
  ...
  fullAutoPresetNameHint: "\u5168\u81EA\u52A8\u6A21\u5F0F\u9884\u8BBE\u952E\uFF08\u9ED8\u8BA4 auto-full\uFF09\uFF1ALLM \u88C1\u51B3\u4E3A\u6700\u7EC8\u51B3\u5B9A\uFF0C\u4E0D\u518D\u4EBA\u5DE5\u5F39\u7A97\u515C\u5E95",
...
var en = {
  ...
  fullAutoPresetNameHint: "Full-auto preset key (default auto-full): the LLM decision is final, no human fallback prompt",
...
// src/client.tsx
var CSS = `.sa_form{flex-direction:column;display:flex}
...
var SafeAutoCardController = class {
  ...
  modelCatalog = null;
  constructor(settingsSource, llmApi, sessionApi) {
    this.llmApi = llmApi;
  ...
  async fetchModelCatalogGroups() {
    if (typeof this.sessionApi?.modelCatalog !== "function") return null;
    try {
      const response = await this.sessionApi.modelCatalog();
      return response?.ok === true && response.value !== null && typeof response.value === "object" ? response.value : null;
    } catch {
      return null;
    }
  }
  ...
  async fetchModelsByDiscovery(provider) {
...
function SafeAutoCard(props) {
  const { t, view } = props;
  const state = props.useSafeAutoCard((snapshot2) => snapshot2);
  const [catalogTick, setCatalogTick] = (0, import_react.useState)(0);
  ...
  return (0, import_jsx_runtime.jsxs)("div", {
    className: "sa_form",
    children: [
      !state.writable || state.dirty ? (0, import_jsx_runtime.jsxs)("div", {
...
function TrailPanel(props) {
  ...
  const snapshot2 = useTrail((s) => s) ?? {};
  if (snapshot2.enabled === false) return null;
  const records = Array.isArray(snapshot2.records) ? snapshot2.records : [];
        ...
        children: [
          (0, import_jsx_runtime.jsxs)("div", {
            ...
            children: [
              (0, import_jsx_runtime.jsx)("button", {
                type: "button",
                className: "sa_trailTab" + (snapshot2.showAll ? "" : " sa_trailTab--active"),
...
var inject = ["slots", "locale", "connection", "sessions", "uiSession", "remote", "remote.session", "remote.settings", "remote.llm"];
function apply(ctx) {
  injectCss();
  ctx.effect(() => ctx.locale.register(SETTINGS_NS, { zh, en }), "autogate: card dictionaries");
  const t = ctx.locale.bind(SETTINGS_NS);
  const trailFetcher = {
    call: async (_namespace, _method, payload) => {
