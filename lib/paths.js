 ...
 */
export function resolveRealPath(path, resolveReal = nativeRealPath) {
...
export function globStaticPrefix(target) {
...
export function isSensitiveConfigFile(target, roots) {
    const normalized = normalizePath(target, roots.workspace, roots.home);
    const real = roots.resolveReal(normalized);
...
export function hardDestructiveTargetReason(target, roots, locale) {
    const normalized = normalizePath(target, roots.workspace, roots.home);
    // symlink 加固：解析真实落点，防止工作区内 symlink 逃逸到关键路径时被词法判定漏过。
    const real = roots.resolveReal(normalized);
    if (isFilesystemRoot(real))
        return reasonText(locale, `文件系统根 ${real}`, `filesystem root ${real}`);
    if (real === roots.home)
        return reasonText(locale, `用户家目录 ${real}`, `user home root ${real}`);
    if (isWithin(roots.dshHome, real))
        return reasonText(locale, `DSH_HOME 路径 ${real}`, `DSH_HOME path ${real}`);
    if (isCriticalPath(real, roots))
        return reasonText(locale, `系统或凭据关键路径 ${real}`, `system or credential-critical path ${real}`);
    return undefined;
}
/** 解析运行时根路径。 */
export function resolveRoots(activeWorkspace, options = {}, resolveReal = resolveRealPath) {
    ...
    const dshHome = resolveReal(normalizePath(options.dshHome ?? (envDshHome === '' || envDshHome === undefined ? posix.join(home, '.dsh') : envDshHome), workspace, home));
    ...
    return { workspace, home, dshHome, tempRoots, resolveReal };
}
