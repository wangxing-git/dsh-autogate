...
export function hardDenyReason(exec, roots) {
    ...
    if (DESTRUCTIVE_TOOL_NAME.test(name)) {
        ...
        if (path !== undefined) {
            const reason = hardDestructiveTargetReason(path, roots);
            if (reason !== undefined)
                return 'destructive tool targets ' + reason;
        }
    }
    return undefined;
}
 ...
 */
export function assessTool(exec, roots) {
    ...
    if (READ_TOOLS.has(name)) {
        const path = pathArgument(args);
        if (path === undefined)
            return allow('read-only project inspection');
        const normalized = normalizePath(path, roots.workspace, roots.home);
        if (isWithin(roots.workspace, normalized))
            return allow('read-only project inspection inside the workspace');
    ...
    if (name === 'write' || name === 'edit' || name === 'apply_patch') {
        const path = pathArgument(args);
        if (path === undefined)
            return allow(name + ' target path is missing; workspace-write sandbox applies');
        const normalized = normalizePath(path, roots.workspace, roots.home);
    ...
    if (SESSION_STATE_TOOLS.has(name))
        return allow('trusted Harness session-state operation');
    if (HARNESS_READ_TOOLS.has(name))
        return allow('trusted read-only Harness operation');
    if (ORCHESTRATION_TOOLS.has(name))
        return allow('orchestration call; child tool actions remain independently checked');
    if (CODE_EXECUTION_TOOLS.has(name))
        return allow('code execution container; inner tool calls are independently checked by policy and sandbox');
    if (EXTERNAL_WRITE_TOOLS.has(name))
        return classify('external write requires specific user authorization: ' + name);
    if (DESTRUCTIVE_TOOL_NAME.test(name)) {
