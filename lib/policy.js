import { hardDestructiveTargetReason, isCriticalPath, isProtectedProjectPath, isSensitiveConfigFile, isWithin, normalizePath } from './paths.js';
...
function containsCredentialMaterial(argumentsValue) {
    ...
    // 按「真实凭据形态」判定，避免 web_search query / send_email body 等自然语言字段里
    // 「Bearer + 英文单词」「sk- + 英文单词」被误拦：
    // - Bearer 值长度 >= 20 且含数字或符号（JWT 含点、随机 token 含数字；英文单词是纯字母）；
    // - sk- / ghp_ / github_pat / xox* 前缀 token 长度 >= 12 且含数字。
    // 纯字母长 token 可能漏检，交 L1 语义审查与沙箱兜底。
    return /(?:BEGIN (?:RSA |OPENSSH )?PRIVATE KEY|\b(?:sk|ghp|github_pat|xox[baprs])[-_](?=[A-Za-z0-9_-]*\d)(?=[A-Za-z0-9_-]{12,})[A-Za-z0-9_-]+|Bearer\s+(?=[A-Za-z0-9._~+\/-]*[0-9._~+\/-])(?=[A-Za-z0-9._~+\/-]{20})[A-Za-z0-9._~+\/-]+|\.ssh[\\/](?:id_|config)|\.credentials\.yaml)/i.test(serialized);
}
/** 同步硬 deny：供 ctx.tools.guard() 与 pre-execute 共用，后续监听器无法覆盖。 */
export function hardDenyReason(exec, roots, locale, mode) {
    ...
    if (['write', 'edit', 'apply_patch'].includes(name)) {
        ...
        if (path !== undefined) {
            ...
            if (reason !== undefined)
                return reasonText(locale, '变更目标为 ', 'mutation targets ') + reason + noEscalationHint(locale);
        }
    }
    if (DESTRUCTIVE_TOOL_NAME.test(name)) {
        ...
        if (path !== undefined) {
            ...
            if (reason !== undefined)
                return reasonText(locale, '破坏性工具目标为 ', 'destructive tool targets ') + reason + noEscalationHint(locale);
        }
    }
    return undefined;
}
 ...
 */
export function assessTool(exec, roots, locale, mode) {
    ...
    if (READ_TOOLS.has(name)) {
        const path = pathArgument(args);
        if (path === undefined)
            return allow(reasonText(locale, '只读项目检查', 'read-only project inspection'));
        ...
        if (isWithin(roots.workspace, real))
            return allow(reasonText(locale, '工作区内只读项目检查', 'read-only project inspection inside the workspace'));
    ...
    if (name === 'write' || name === 'edit' || name === 'apply_patch') {
        const path = pathArgument(args);
        if (path === undefined)
            return allow(name + reasonText(locale, ' 目标路径缺失；workspace-write 沙箱适用', ' target path is missing; workspace-write sandbox applies'));
        const normalized = normalizePath(path, roots.workspace, roots.home);
    ...
    if (SESSION_STATE_TOOLS.has(name))
        return allow(reasonText(locale, '受信任的 Harness 会话状态操作', 'trusted Harness session-state operation'));
    if (HARNESS_READ_TOOLS.has(name))
        return allow(reasonText(locale, '受信任的 Harness 只读操作', 'trusted read-only Harness operation'));
    if (ORCHESTRATION_TOOLS.has(name))
        return allow(reasonText(locale, '编排调用；子工具动作仍被独立检查', 'orchestration call; child tool actions remain independently checked'));
    if (CODE_EXECUTION_TOOLS.has(name))
        return allow(reasonText(locale, '代码执行容器；内部工具调用由策略与沙箱独立检查', 'code execution container; inner tool calls are independently checked by policy and sandbox'));
    if (EXTERNAL_WRITE_TOOLS.has(name))
        return classify(reasonText(locale, '外部写入需用户明确授权：', 'external write requires specific user authorization: ') + name);
    if (DESTRUCTIVE_TOOL_NAME.test(name)) {
...
export function summarizeToolArguments(name, args) {
...
export function hasSandboxEscalation(args) {
    const parsed = record(args);
    if (parsed === undefined)
        return false;
...
export function isSandboxEscalationRetry(name, args) {
    if (name !== 'bash' && name !== 'pwsh')
        return false;
