...
export const CLASSIFIER_SYSTEM_PROMPT = [
    ...
    '5. DENY genuinely dangerous operations: exfiltrating credentials or sensitive data; reading private keys or credential material (e.g. .ssh/id_* WITHOUT a .pub extension, .aws/credentials, .gnupg, .env) when the stated task does not require it; bypassing the permission system or sandbox; and destructive or irreversible changes to critical targets — overwriting or deleting shell/credential configs, system directories, the user home ROOT itself, or the filesystem root. Reading a public key (*.pub), or reading your own credential file when a clearly stated legitimate task requires it, is NOT automatically a deny — judge by the concrete target and stated task.',
    '',
    'STEP 4 — Use policyReason. The "policyReason" field is the deterministic-layer preliminary risk assessment. Treat it as a risk hint, not a verdict: when it flags a specific danger, require stronger evidence of safety or explicit authorization before allowing.',
    '',
