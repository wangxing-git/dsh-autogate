 ...
 *
 * `agent` 必须是**发起该调用的执行 agent**（approval/request 的 `req.agent`），不是授权解析得到的
 * 祖先会话：子代理的 tool/call 落在子代理会话，查祖先会话查不到（曾据 authorization 传参导致回退失效）。
 *
...
export function apply(ctx, config = {}) {
    ...
    ctx.on('approval/request', async (req, next) => {
        ...
        try {
            ...
            // 回退查的是**执行会话**（req.agent）而非授权会话（authority.agent）：tool/call 事件落在实际发起调用的
            // 会话里，子代理场景下二者不同（授权会话是其祖先、事件里没有这次调用），查授权会话必然落空。
            const rawArguments = pendingApprovalArgs.get(callId)
                ?? toolCallArgumentsFromEvents(req.agent, callId)
                ?? { reason };
