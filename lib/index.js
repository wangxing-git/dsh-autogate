...
async function readTrailPayload(request) {
    try {
 ...
 * 只用于让子代理会话的权限档投影（UI 显示）与父会话一致，不是用户授权：
 * 授权解析（isAutoPermissionExecution / managedPermissionAuthority）必须跳过该标记，
 * 沿 durable parentSession 链把授权依据锚定在顶层 Auto 会话（唯一持有直接人类消息的会话）。
 */
 ...
 */
export function applyAuthorizationPreset(state, event) {
...
export function isAutoPermissionExecution(exec, resolvePreset, presetName = AUTO_PERMISSION_PRESET) {
 ...
 */
export function autoPermissionAuthority(exec, parentAgent, resolvePreset, presetName = AUTO_PERMISSION_PRESET) {
    if (isAutoPermissionExecution(exec, resolvePreset, presetName))
        return exec.agent;
    ...
    while (session?.header?.origin === 'subagent' && session.header.parentSession !== undefined) {
        ...
        if (isAutoPermissionExecution(parentExec, resolvePreset, presetName))
            return parent;
...
export function managedPermissionAuthority(agent, parentAgent, resolvePreset, semiPreset = SEMI_AUTO_PERMISSION_PRESET, fullPreset = AUTO_PERMISSION_PRESET) {
    const modeOf = (target) => {
...
/**
 * tool/call 参数缓存（执行会话侧）容量上限，与 pendingApprovalArgs 同量级：
 * 调用正常完成时条目不会被审批取回，需有界淘汰避免进程级内存增长。
 */
const TOOL_CALL_ARGUMENTS_LIMIT = 100;
/**
 ...
 */
export function rememberToolCallArguments(cache, event) {
    ...
    cache.set(String(event.data.callId), parsed);
    if (cache.size > TOOL_CALL_ARGUMENTS_LIMIT) {
...
export function apply(ctx, config = {}) {
    ...
    // 无法逐次读取，故配置变化后必须整体重建）。
    let presetName = SEMI_AUTO_PERMISSION_PRESET;
    let fullAutoPresetName = AUTO_PERMISSION_PRESET;
    let classifier;
    let rootOptions = {};
    let preflight = false;
    ...
    // 参数回退缓存（callId → 已解析参数），由 session/event 增量维护。pre-execute 监听器被更早注册的
    // 监听器短路时 pendingApprovalArgs 不会写入，此时按 callId 从本缓存取回参数，分类器仍能评估具体目标。
    //
    // 之所以用订阅而非扫描会话事件（DSH 0.1.6 起同步事件读取已弃用）：查询目标恒为「当前正在审批的
    // 调用」，其 tool/call 事件必然在本插件加载之后 append，`session/event` 为 append 内的同步派发、
    // 覆盖全部会话，故订阅即可完整覆盖；判定成本同时由 O(日志长度) 降为 O(1)。
    // 缓存按 callId 存放、不区分会话，故子代理场景（授权会话是执行会话的祖先）同样命中。
    const toolCallArguments = new Map();
    ctx.on('session/event', (_session, event) => { rememberToolCallArguments(toolCallArguments, event); });
    ...
    const authorityOf = (target) => managedPermissionAuthority(target, parentAgent, resolvePreset, presetName, fullAutoPresetName);
    const authorityFor = (exec) => authorityOf(exec.agent);
    ...
    ctx.on('session/created', (session) => {
        ...
        session.append('permission/preset', {
            preset: authority.mode === 'semi-auto' ? presetName : fullAutoPresetName,
            source: INHERITED_PRESET_SOURCE,
        });
    ...
    ctx.on('approval/request', async (req, next) => {
        ...
        try {
            ...
            // 优先用 pre-execute 缓存；未命中（如工具 pre-execute 监听器短路了本插件）时取 session/event
            // 订阅维护的 tool/call 参数缓存；仍无则用 reason 兜底（分类器信息不足时倾向 fail-closed）。
            const rawArguments = pendingApprovalArgs.get(callId)
                ?? toolCallArguments.get(callId)
                ?? { reason };
