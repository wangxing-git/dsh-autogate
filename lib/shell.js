...
export function hardDenyShellReason(source, _shell, _roots) {
    const compact = source.trim();
    if (PRIVILEGE_ESCALATION.test(compact))
        return 'privilege escalation is not permitted by auto mode';
    if (SELF_DESTRUCTIVE.test(compact))
        return 'self-destructive or system-level command is not permitted';
    ...
    if (/rm\s+(?:-[a-z]*[fr][a-z]*\s+)*\/(?:\s|$)/.test(compact))
        return 'deleting the filesystem root is not permitted';
    if (/(?:rm|Remove-Item)\s+(?:-[a-z]*[fr][a-z]*\s+)*(?:~|\$HOME|\$env:HOME)(?:\s|$)/i.test(compact))
        return 'deleting the user home root is not permitted';
    return undefined;
}
 ...
 */
function assessDestructive(name, tokens, roots) {
    ...
    if (targets.length === 0)
        return allow('destructive target could not be determined; workspace-write sandbox applies');
    for (const target of targets) {
        // glob 目标：先对其静态前缀做危险判定，防止 /*、/etc/*、~/*、~/.* 这类绕过精确路径匹配。
        const reason = hardDestructiveTargetReason(globStaticPrefix(target), roots);
        if (reason !== undefined)
            return deny('destructive operation targets ' + reason);
    }
    // dd 是块设备级操作，参数形如 if=/of=，静态路径判定不可靠，交 LLM 分类。
    if (name === 'dd')
        return classify('dd block-device operation requires independent classification');
    ...
    if (confined)
        return allow('destructive operation confined to the workspace or temporary area; workspace-write sandbox applies');
...
export function assessShell(source, shell, roots) {
    ...
    if (rawName === '')
        return allow('command line contains no command');
    if (/[\$\x60]/.test(rawName))
        return allow('command name is produced by a dynamic expansion; workspace-write sandbox applies');
    const name = commandName(rawName);
    // 提权/自毁命令：即使原始正则被引号拼接（如 s'u'do）绕过，tokenize 后仍能确定性拒绝。
    if (PRIVILEGE_ESCALATION_COMMANDS.has(name))
        return deny('privilege escalation is not permitted by auto mode');
    if (SELF_DESTRUCTIVE_COMMANDS.has(name))
        return deny('self-destructive or system-level command is not permitted');
    ...
    if (DESTRUCTIVE_COMMANDS.has(name))
        return assessDestructive(name, tokens, roots);
    if (name === 'find') {
        if (/-(?:delete|exec|execdir|ok|okdir)\b/.test(compact))
            return classify('find with a mutating action requires independent classification');
    ...
    if (isBuildOrTest(name, tokens))
        return allow('recognized project build, test, or verification command');
