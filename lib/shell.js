...
export function hardDenyShellReason(source, _shell, _roots, locale) {
    const compact = source.trim();
    if (PRIVILEGE_ESCALATION.test(compact))
        return reasonText(locale, 'auto 模式不允许提权', 'privilege escalation is not permitted by auto mode');
    if (SELF_DESTRUCTIVE.test(compact))
        return reasonText(locale, '不允许自毁或系统级命令', 'self-destructive or system-level command is not permitted');
    if (NETWORK_COMMAND.test(compact) && SENSITIVE_MARKER.test(compact))
        return reasonText(locale, '不允许凭据或私密数据外传', 'credential or private-data exfiltration pattern is not permitted');
    if (/rm\s+(?:-[a-z]*[fr][a-z]*\s+)*\/(?:\s|$)/.test(compact))
        return reasonText(locale, '不允许删除文件系统根', 'deleting the filesystem root is not permitted');
    if (/(?:rm|Remove-Item)\s+(?:-[a-z]*[fr][a-z]*\s+)*(?:~|\$HOME|\$env:HOME)(?:\s|$)/i.test(compact))
        return reasonText(locale, '不允许删除用户家目录', 'deleting the user home root is not permitted');
    return undefined;
}
...
function assessDestructive(name, tokens, roots, locale) {
    ...
    for (const target of targets) {
        ...
        if (reason !== undefined)
            return deny(reasonText(locale, '破坏性操作目标为 ', 'destructive operation targets ') + reason);
    }
...
export function assessShell(source, shell, roots, locale) {
    ...
    if (PRIVILEGE_ESCALATION_COMMANDS.has(name))
        return deny(reasonText(locale, 'auto 模式不允许提权', 'privilege escalation is not permitted by auto mode'));
    if (SELF_DESTRUCTIVE_COMMANDS.has(name))
        return deny(reasonText(locale, '不允许自毁或系统级命令', 'self-destructive or system-level command is not permitted'));
