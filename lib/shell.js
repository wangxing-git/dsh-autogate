...
function networkSegments(source) {
    const segments = [];
    for (const segment of commandSegments(source)) {
        const tokens = tokenize(segment);
        const name = commandName(tokens[0] ?? '');
        if (name !== '' && NETWORK_COMMAND_NAMES.has(name))
 ...
 */
function exfiltrationReason(source, locale) {
    ...
    for (const segment of segments) {
        if (SENSITIVE_FILE_REFERENCE.test(segment))
            return denied;
    }
    if (!segments.some((segment) => SENSITIVE_TEXT_MARKER.test(segment)))
        return undefined;
    const hosts = [];
    for (const segment of segments) {
    ...
    if (hosts.length > 0 && hosts.every((host) => isLoopbackHost(host)))
        return undefined;
    return denied;
}
